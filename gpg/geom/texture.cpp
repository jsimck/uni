...
Texture *LoadTexture(const char *file_name, const int flip, const bool single_channel) {
    ...
    cv::Mat image_rgba = cv::Mat(image_bgr.size(),
                                 image_bgr.depth(), (single_channel) ? 1 : 4);

    if (image_bgr.channels() == 4) {
        // obr�zek je u� na�ten i s alfa kan�lem
