...
const char Camera::scanline_hilbert_4x4[] = {0, 0, 1, 0, 1, 1, 0, 1, 0, 2, 0, 3, 1, 3, 1, 2, 2, 2, 2, 3, 3, 3, 3, 2, 3,
                                             1, 2, 1, 2, 0, 3, 0};
...
Vector3 Camera::ws2es(const Vector3 &p) const {
    return view_ * Vector4(p); // přechod bodu ze světového souřadného systému (world space) do kamerového (eye space)
}
...
Vector3 Camera::normal_ws2es(const Vector3 &p) const {
    ...
    return normal_es; // přechod normály ze světového souřadného systému (world space) do kamerového (eye space)
}
...
void Camera::Update() {
    ...
    pixel_size_ = 2 * d_ * tan(fov_y_ * 0.5f) / height_;
}
...
Ray Camera::GenerateRay(const float sx, const float sy) const {
    ...
    direction = view_t_ * direction; // p�echod do sv�tov�ho sou�adn�ho syst�mu
    direction.Normalize();
...
void Camera::Rotate(float angle) {
    ...
    // Move view from back to origin
    view_from_.x -= view_at_.x;
    view_from_.y -= view_at_.y;

    // Rotate point
    float xnew = view_from_.x * cn - view_from_.y * sn;
    float ynew = view_from_.x * sn + view_from_.y * cn;

    // Move view from back to original offset
    view_from_.x = xnew + view_at_.x;
    view_from_.y = ynew + view_at_.y;
    Update();
}
...
void Camera::ResetPosition() {
    view_from_ = Vector3(view_from_backup_);
    Update();
}
