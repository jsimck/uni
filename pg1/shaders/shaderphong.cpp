...
Color4 ShaderPhong::trace(Ray &ray, int depth) {
    ...
    if (!ray.hasCollided()) {
        return this->cubeMap != NULL ? this->cubeMap->getTexel(dir) : Color4(0);
    }
    ...
    if (depth <= 0) {
        return this->light.ambient + calcColor(ray, mat, normal) * mat->reflectivity * specular;
    }

    // Compute refracted ray
    if (mat->get_name() == "green_plastic_transparent" || mat->get_name() == "wire_214229166") {
    ...
    return this->light.ambient + calcColor(ray, mat, normal) + trace(reflectedRay, depth - 1) * mat->reflectivity * specular;
}

Color4 ShaderPhong::calcColor(Ray &ray, Material *mat, Vector3 &normal) {
    return (calcDiffuse(ray, normal) * getDiffuse(ray) * calcShadow(ray)) * (1.0f - mat->reflectivity);
}
