...
#define RENDER_TYPE_PATH 2
#define RENDER_TYPE_PHONG 3

int main(int argc, char *argv[]) {
    int renderModel = RENDER_MODEL_ALIEN;
    int renderType = RENDER_TYPE_PATH;
    ...
    if (renderModel == RENDER_MODEL_SPHERE) {
        if (LoadOBJ("data/geosphere.obj", defaultColor, surfaces, materials) < 0) {
            return -1;
        }
    } else {
...
//    Shader *shader = new ShaderNormal(camera, scene, surfaces, materials, nullptr);
//    Shader *shader = new ShaderLambert(camera, scene, surfaces, materials, nullptr);
    if (renderType == RENDER_TYPE_PHONG) {
        shader = new ShaderPhong(camera, scene, surfaces, materials, light, cubeMap);
    } else if (renderType == RENDER_TYPE_PATH) {
        shader = new ShaderPathTracer(camera, scene, surfaces, materials, light, nullptr);
    }
